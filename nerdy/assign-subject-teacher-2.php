...
<div class="d-flex container-fluid">
    ...
    <div class="school-main-dashboard container section-container mb-5 animate__animated animate__fadeIn">
        <div class="mb-5 w-100">
            ...
            <div class="card p-3 w-100">
                ...
                <form action="" method="POST">
                    <div class="mob-flex d-flex">
                        ...
                        <!-- Subject -->
                        <select required class="form-select equal-flex m-1" name="teacher_assigned"
                        ...
                        <!-- Subject -->
                        <select required class="form-select equal-flex m-1" name="teacher_assigned_subject"
                            aria-label="Default select example">
                        ...
                        <!-- Class -->
                        <select required class="form-select equal-flex m-1" name="teacher_assigned_class"
                            aria-label="Default select example">
            ...
            <div class="mob-flex d-flex w-100 mt-4">
                <form action="assign-subject-teacher-2.php" method="POST" class="col-md-4 mb-3 filter-card">
                    <div class="form-floating">
                        <select class="form-select" name="teacher_assigned" id="floatingSelect"
                            aria-label="Floating label select example">
                            <option selected>Filter Teacher</option>
                            <?php
                            $get_teacher_data = "SELECT * FROM `users` WHERE `user_added_by` = $session_user_id AND `user_type` = 3 OR `user_type` = 5";
                            $get_teacher_data_res = mysqli_query($connection, $get_teacher_data);
                            while ($row = mysqli_fetch_assoc($get_teacher_data_res)) {
                                $user_id = $row['user_id'];
                                $user_name = $row['user_name']; ?>
                            <option value="<?php echo $user_id ?>"><?php echo $user_name ?></option>
                            <?php } ?>
                        </select>
                        <label for="floatingSelect">Click here to get Teacher list</label>
                    </div>
                    <div class="mt-3">
                        <button type="submit" name="filter" class="w-100 btn btn-outline-primary">Filter</button>
                    </div>
                </form>
                <div class="col-md-8" class="assigned-table">
